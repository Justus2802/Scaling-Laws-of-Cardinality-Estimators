...
### Star counts are exact via inclusion-exclusion over neighbourhood edges
`count_stars_exact` counts induced k-stars exactly for k=2..10. For each node v with degree d, the count equals the number of size-k independent sets in N(v). When N(v) has no internal edges (a pure star), every k-subset qualifies: C(d, k). Otherwise, inclusion-exclusion over the edge set E_v inside N(v) subtracts overcounted subsets:

    stars_k(v) = C(d,k) - Σ_{F⊆E_v, F≠∅} (-1)^{|F|+1} · C(d-|V(F)|, k-|V(F)|)

where V(F) is the endpoint set of edge-subset F. The sum iterates over all 2^|E_v| non-empty subsets of internal edges. For KGs this is fast because neighbourhoods are sparse (|E_v| is small). Nodes with degree > 50 (HUB_THRESH) fall back to direct k-subset enumeration with an independence check — tractable at MAX_K=10.

The previous C(d,k) formula was incorrect for nodes whose neighbours have edges between them (e.g. nodes in a triangle counted 1 induced 2-star, but C(2,2)=1 even though both neighbours are connected, so the result was wrong). The new implementation is verified against pure stars, triangles, K4, hub+leaf-edge, C5, and the Petersen graph.
