"""Measure the reduced graph signature of a KG file and write results to a directory.

Sibling of ``measure_signature.py`` for the reduced (non-over-determined)
signature (Blocks A, B, C, D, F). Writes to ``sig_out_reduced/<name>_signature/``
so it never clobbers the original signature's output under ``sig_out/``.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory to write results into (default: sig_out_reduced/<graph name>_signature/)",
    )
    parser.add_argument(
    ...
    out_dir = Path(args.output_dir) if args.output_dir else Path("sig_out_reduced") / f"{graph_name}_signature"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"Loading  : {args.kg_file}")
    sig = compute_reduced_signature(args.kg_file, verbose=True, blocks=selected_blocks)

    all_blocks = [("a", sig.a), ("b", sig.b), ("c", sig.c), ("d", sig.d), ("f", sig.f)]
